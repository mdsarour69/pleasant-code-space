...
function ServicesTab({ services }: { services: Service[] }) {
  ...
  const save = useMutation({ mutationFn: (value: Parameters<typeof saveService>[0]) => saveFn({ data: value }), onSuccess: () => { toast.success("Service saved"); setForm(null); void refresh(); }, onError: fail });
...
function PackagesTab({ packages }: { packages: Pkg[] }) {
  ...
  const refresh = () => qc.invalidateQueries({ queryKey: ["admin-data"] }); const fail = (e: unknown) => toast.error(errorMessage(e));
  const save = useMutation({ mutationFn: (value: Parameters<typeof savePackage>[0]) => saveFn({ data: value }), onSuccess: () => { toast.success("Package saved"); setForm(null); void refresh(); }, onError: fail });
