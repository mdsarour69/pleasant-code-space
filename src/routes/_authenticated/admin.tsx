...
const throwIfError = (error: { message: string } | null) => { if (error) throw error; };
...
async function fetchAdminData(): Promise<AdminData> {
  ...
  throwIfError(services.error ?? packages.error ?? orders.error ?? settings.error);
  return { services: services.data ?? [], packages: packages.data ?? [], orders: orders.data ?? [], settings: settings.data ?? [] };
}
...
function AdminPage() {
  ...
  const roleQuery = useQuery({ queryKey: ["my-role"], queryFn: fetchAdminRole, enabled: sessionReady && hasSession, retry: 1, staleTime: 60_000 });
  const dataQuery = useQuery({ queryKey: ["admin-data"], queryFn: fetchAdminData, enabled: roleQuery.data?.isAdmin === true, retry: 1, staleTime: 15_000 });
...
function OrdersTab({ orders }: { orders: Order[] }) {
  const qc = useQueryClient(); const [filter, setFilter] = useState("all");
  const settle = () => qc.invalidateQueries({ queryKey: ["admin-data"] }); const fail = (e: unknown) => toast.error(errorMessage(e));
  const del = useMutation({ mutationFn: async (id: string) => { const { error } = await supabase.from("orders").delete().eq("id", id); throwIfError(error); }, onSuccess: () => { toast.success("Order deleted"); void settle(); }, onError: fail });
  const delAll = useMutation({ mutationFn: async () => { const { error } = await supabase.from("orders").delete().neq("id", "00000000-0000-0000-0000-000000000000"); throwIfError(error); }, onSuccess: () => { toast.success("All orders deleted"); void settle(); }, onError: fail });
  const status = useMutation({ mutationFn: async ({ id, value }: { id: string; value: "pending" | "confirmed" | "done" | "cancelled" }) => { const { error } = await supabase.from("orders").update({ status: value }).eq("id", id); throwIfError(error); }, onSuccess: () => { toast.success("Status updated"); void settle(); }, onError: fail });
...
function ServicesTab({ services }: { services: Service[] }) {
  const qc = useQueryClient(); const [form, setForm] = useState<Partial<Service> | null>(null); const [lang, setLang] = useState<LangCode>("bn");
  const refresh = () => qc.invalidateQueries({ queryKey: ["admin-data"] }); const fail = (e: unknown) => toast.error(errorMessage(e));
  const save = useMutation({ mutationFn: async (value: TablesInsert<"services"> & { id?: string }) => { const { id, ...fields } = value; const result = id ? await supabase.from("services").update(fields as TablesUpdate<"services">).eq("id", id) : await supabase.from("services").insert(fields); throwIfError(result.error); }, onSuccess: () => { toast.success("Service saved"); setForm(null); void refresh(); }, onError: fail });
  const del = useMutation({ mutationFn: async (id: string) => { const { error } = await supabase.from("services").delete().eq("id", id); throwIfError(error); }, onSuccess: () => { toast.success("Service deleted"); void refresh(); }, onError: fail });
  const delAll = useMutation({ mutationFn: async () => { const { error } = await supabase.from("services").delete().neq("id", "00000000-0000-0000-0000-000000000000"); throwIfError(error); }, onSuccess: () => { toast.success("All services deleted"); void refresh(); }, onError: fail });
   const submit = () => { if (!form?.title?.trim()) { toast.error("Bengali/default title is required"); return; } const values: TablesInsert<"services"> = { icon: form.icon ?? "", gradient: form.gradient ?? "pink", active: form.active ?? true, sort_order: form.sort_order ?? 0, title: form.title, description: form.description ?? "", title_en: form.title_en ?? "", title_ar: form.title_ar ?? "", title_fr: form.title_fr ?? "", title_pt: form.title_pt ?? "", description_en: form.description_en ?? "", description_ar: form.description_ar ?? "", description_fr: form.description_fr ?? "", description_pt: form.description_pt ?? "" }; save.mutate(form.id ? { ...values, id: form.id } : values); };
...
function PackagesTab({ packages }: { packages: Pkg[] }) {
  const qc = useQueryClient(); const [form, setForm] = useState<Partial<Pkg> | null>(null); const [lang, setLang] = useState<LangCode>("bn");
  const refresh = () => qc.invalidateQueries({ queryKey: ["admin-data"] }); const fail = (e: unknown) => toast.error(errorMessage(e));
  const save = useMutation({ mutationFn: async (value: TablesInsert<"packages"> & { id?: string }) => { const { id, ...fields } = value; const result = id ? await supabase.from("packages").update(fields as TablesUpdate<"packages">).eq("id", id) : await supabase.from("packages").insert(fields); throwIfError(result.error); }, onSuccess: () => { toast.success("Package saved"); setForm(null); void refresh(); }, onError: fail });
  const del = useMutation({ mutationFn: async (id: string) => { const { error } = await supabase.from("packages").delete().eq("id", id); throwIfError(error); }, onSuccess: () => { toast.success("Package deleted"); void refresh(); }, onError: fail }); const delAll = useMutation({ mutationFn: async () => { const { error } = await supabase.from("packages").delete().neq("id", "00000000-0000-0000-0000-000000000000"); throwIfError(error); }, onSuccess: () => { toast.success("All packages deleted"); void refresh(); }, onError: fail });
   const submit = () => { if (!form?.name?.trim()) { toast.error("Bengali/default name is required"); return; } const values: TablesInsert<"packages"> = { price: form.price ?? 0, old_price: form.old_price ?? 0, active: form.active ?? true, sort_order: form.sort_order ?? 0, name: form.name, duration: form.duration ?? "", badge: form.badge ?? "", type: form.type ?? "", description: form.description ?? "", button: form.button ?? "BUY NOW", name_en: form.name_en ?? "", name_ar: form.name_ar ?? "", name_fr: form.name_fr ?? "", name_pt: form.name_pt ?? "", duration_en: form.duration_en ?? "", duration_ar: form.duration_ar ?? "", duration_fr: form.duration_fr ?? "", duration_pt: form.duration_pt ?? "", badge_en: form.badge_en ?? "", badge_ar: form.badge_ar ?? "", badge_fr: form.badge_fr ?? "", badge_pt: form.badge_pt ?? "", type_en: form.type_en ?? "", type_ar: form.type_ar ?? "", type_fr: form.type_fr ?? "", type_pt: form.type_pt ?? "", description_en: form.description_en ?? "", description_ar: form.description_ar ?? "", description_fr: form.description_fr ?? "", description_pt: form.description_pt ?? "" }; save.mutate(form.id ? { ...values, id: form.id } : values); };
...
function BrandingTab({ settings }: { settings: Setting[] }) {
  ...
  const applyTemplate = useMutation({
    mutationFn: async (values: Record<string, string>) => { const { error } = await supabase.from("settings").upsert(Object.entries(values).map(([key, value]) => ({ key, value })), { onConflict: "key" }); throwIfError(error); },
