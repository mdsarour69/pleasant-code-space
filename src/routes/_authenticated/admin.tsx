...
import {
  ...
  getMyRole, savePackage, saveService, saveSetting, saveSettings, updateOrderStatus,
...
function ServicesTab({ services }: { services: Service[] }) {
  ...
  const save = useMutation({ mutationFn: (value: Record<string, unknown>) => saveFn({ data: value as Parameters<typeof saveFn>[0]["data"] }), onSuccess: () => { toast.success("Service saved"); setForm(null); void refresh(); }, onError: fail });
...
function PackagesTab({ packages }: { packages: Pkg[] }) {
  ...
  const refresh = () => qc.invalidateQueries({ queryKey: ["admin-data"] }); const fail = (e: unknown) => toast.error(errorMessage(e));
  const save = useMutation({ mutationFn: (value: Parameters<typeof saveFn>[0]["data"]) => saveFn({ data: value }), onSuccess: () => { toast.success("Package saved"); setForm(null); void refresh(); }, onError: fail });
